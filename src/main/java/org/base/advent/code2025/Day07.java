package org.base.advent.code2025;

import org.apache.commons.lang3.tuple.Pair;
import org.base.advent.ParallelSolution;
import org.base.advent.util.Node;

import java.util.*;
import java.util.concurrent.ExecutorService;
import java.util.function.Function;

/**
 * <a href="https://adventofcode.com/2025/day/7">Day 7</a>
 */
public class Day07 extends ParallelSolution<List<String>> {
    public Day07(ExecutorService pool) {
        super(pool);
    }

    @Override
    public Object solvePart1(List<String> input) {
        Set<Integer> beams = new HashSet<>();
        beams.add(input.get(0).indexOf('S'));
        int split = 0;

        for (int i = 1; i < input.size(); i++) {
            Set<Integer> next = new HashSet<>();
            String row = input.get(i);
            for (int b : beams) {
                if (row.charAt(b) == '^') {
                    next.add(b - 1);
                    next.add(b + 1);
                    split++;
                }
                else
                    next.add(b);
            }
            beams = next;
        }
        return split;
    }

    @Override
    public Object solvePart2(List<String> input) {
        Node<Integer> start = Node.createRootNode(input.get(0).indexOf('S'));
        List<Node<Integer>> beams = new LinkedList<>();
        beams.add(start);
        int split = 0;

        for (int i = 1; i < input.size(); i++) {
            List<Node<Integer>> next = new LinkedList<>();
            String row = input.get(i);
            for (Node<Integer> b : beams) {
                if (row.charAt(b.getData()) == '^') {
                    next.add(b.addChild(b.getData() - 1));
                    next.add(b.addChild(b.getData() + 1));
                }
                else
                    next.add(b);
            }
            beams = next;
        }
        return beams.size();

    }
}
