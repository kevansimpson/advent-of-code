 ...
 * The hash these programs are implementing isn't very strong, but you decide to help them anyway. You make a
 * mental note to remind the Elves later not to invent their own cryptographic functions.
 *
 * This hash function simulates tying a knot in a circle of string with 256 marks on it. Based on the input to be
 * hashed, the function repeatedly selects a span of string, brings the ends together, and gives the span a
 * half-twist to reverse the order of the marks within it. After doing this many times, the order of the marks is
 ...
 * in your puzzle input. Once this process is complete, what is the result of multiplying the first two numbers in the list?
 *
 * <h2>Part 2</h2>
 ...
 * example was your first round, you would start your second round with the same length sequence
 ...
 * of 256 numbers). So, the first element in the dense hash is the first sixteen elements of the sparse hash XOR'd together,
 ...
 * hexadecimal notation. Because each number in your dense hash will be between 0 and 255 (inclusive), always represent each
 * number as two hexadecimal digits (including a leading zero as necessary). So, if your first three numbers are 64, 7, 255,
 * they correspond to the hexadecimal numbers 40, 07, ff, and so the first six characters of the hash would be 4007ff. Because
 * every Knot Hash is sixteen such numbers, the hexadecimal representation is always 32 hexadecimal digits (0-f) long.
 *
